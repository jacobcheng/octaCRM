...
class Follow extends Backend
{
    ...
    /**
     * Follow模型对象
     * @var \app\admin\model\sales\Follow
     */
    ...
    {
        parent::_initialize();
        $this->model = new \app\admin\model\sales\Follow;
    ...
    public function index()
    {
        ...
        if ($this->request->isAjax())
        {
            ...
            $total = $this->model
                    ->with(['client','contact','quotation','order','admin'])
                    ->where($where)
                    ->order($sort, $order)
                    ->count();
            ...
            foreach ($list as $row) {
                $row->visible(['id','client_id','contact_id','quotation_id','order_id','channel','detail','nextdate','admin_id','createtime']);
                $row->visible(['client']);
